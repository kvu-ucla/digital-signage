...
test.describe("vertical screen", () => {
  test.beforeEach(async ({ page }) => {
    await page.goto(
      "/?location=bruinplate&screen=vertical&menu=lunch&station=simply+grilled&mock=true",
    );
  });

  test("renders the station title", async ({ page }) => {
    await expect(page.locator(".screen-vertical__header-title")).toBeVisible();
    await expect(page.locator(".screen-vertical__header-title")).toContainText(
      "Simply Grilled",
    );
  });
  ...
  test("hides the station title in minimal mode", async ({ page }) => {
    await page.goto(
      "/?location=bruinplate&screen=vertical&menu=lunch&station=simply+grilled&mock=true&minimal=true",
    );
